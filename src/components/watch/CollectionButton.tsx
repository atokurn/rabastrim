...
export function CollectionButton({
    ...
    if (variant === 'mobile-sidebar') {
        return (
            <button onClick={handleToggle} className={`flex flex-col items-center gap-1 group ${className}`}>
                ...
                <span className="text-white text-xs font-medium text-shadow">{isCollected ? t('buttons.collection_saved') : t('buttons.collection_add')}</span>
            </button>
        );
    }
    ...
    return (
        ...
        >
            <Bookmark className={`w-5 h-5 ${isCollected ? 'text-[#00cc55] fill-[#00cc55]' : ''}`} />
            <span className="text-xs">{isCollected ? t('buttons.collection_saved') : t('buttons.collection_add')}</span>
        </button>
    );
}
